...
const Comment = ({ comment, user, onReplySubmit }) => {
    ...
    return (
        <div className={`card my-3 ${comment.parent_id !== 0 ? "ml-5" : ""}`}>
            <div className="card-body">
                ...
                {comment.replies.map(reply => (
                    <Comment key={reply.id} comment={reply} user={user} onReplySubmit={onReplySubmit} />
                ))}
            </div>
        </div>
    );
...
const CommentSection = ({ pageId, user }) => {
    ...
    const handleCommentSubmit = async (event) => {
        ...
        const response = await fetch(`/api/comments`, {
            ...
            body: JSON.stringify({ pageId, author: user.username, content: newComment }),
        });
        if (!response.ok) {
    ...
    const handleReplySubmit = async (parentId, replyContent) => {
        ...
        const response = await fetch(`/api/comments`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ pageId, author: user.username, content: replyContent, parent_id: parentId }),
        });
        if (!response.ok) {
    ...
    return (
        <div>
            ...
            <div className="card-body bg-light">
                <h4>{user ? 'Spune părerea ta: ' : 'Doar utilizatorii înregistrați pot lăsa comentarii.'}</h4>
                <form onSubmit={handleCommentSubmit}>
                    ...
                    />
                    <button type="submit" name="create_comment" className="mt-1 btn btn-outline-primary" disabled={!user}>
                        Postează Comentariu
                    </button>
                </form>
            </div>
            ...
            {comments.length > 0 ? (
                <div>
                    ...
                    {comments.map(comment => (
                        <Comment key={comment.id} comment={comment} user={user} onReplySubmit={handleReplySubmit} />
                    ))}
                </div>
            ) : (
